...
function renderSliders(data, htmlType) {
  ...
  data.forEach((popularMovieSerie) => {
    popularAll += `
      <div>
        <img data-id="${popularMovieSerie.id}" data-type=${
      popularMovieSerie.media_type
    } class="movie-image" alt="${
      popularMovieSerie.title
    }" src="https://image.tmdb.org/t/p/w500/${
      popularMovieSerie.poster_path
    }" alt="cyberpunk"/>
        <div class="genre-text">
          <h3>${
            popularMovieSerie.title
              ? popularMovieSerie.title
              : popularMovieSerie.name
          }</h3>
          <p>${
            popularMovieSerie.media_type
          } | Rating: ${popularMovieSerie.vote_average.toFixed(1)}</p>
        </div>
      </div>
      `;
  ...
  if (htmlType === popularTv) {
    ...
    randomPopular.forEach((slider) => {
      hero += `
        <div class="movie">
          <img class="movie-background" src="https://image.tmdb.org/t/p/original/${
            slider.backdrop_path
          }" alt="background">
          <div class="movie-text">
            ...
            <button data-id="${slider.id}" data-type=${
        slider.media_type
      } class="play-btn">Play<i class="fa-solid fa-play"></i></button><button class="list-btn">Add to Wishlist<i class="fa-solid fa-clipboard-list"></i></button>
          </div>
        </div>
        `;
